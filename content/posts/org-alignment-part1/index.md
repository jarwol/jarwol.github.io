...
How about a less contrived example? Imagine a streaming company that has a goal of decreasing subscriber churn rate. The Analytics team found that subscribers who take more than a minute to find content to play are much more likely to churn than those who find their content quickly after opening the app. They also found that the vast majority of subscribers look for content by browsing through the top 10 rows of tiles on the home screen while relatively few leverage search. The Content Engineering team gets together with the Customer Success team and finds out there’s been an uptick in customer complaints about search relevance, so they work with Data Science to design and implement enhanced LLM-based search. That’s an example of poorly aligned priorities. In a vacuum, it’s great that the team wants to improve the product and address customer complaints, but that’s not what the organizational and company strategy is targeting, so it’s unlikely to lead to an outcome that leadership (and potentially shareholders) are happy with. It doesn’t matter if the Content Engineering team is firing on all cylinders and shipping code faster than any other team in the company. They’re moving in the wrong direction.
